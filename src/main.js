...
function render() {
  ...
  <div class="noise"></div><main class="shell">
    ...
    <section class="section projects" id="projects"><div class="section-heading reveal"><div class="section-title"><i></i><h2>${t.projectsTitle}</h2></div><span>${t.projectsHint}</span></div><div class="project-grid">${activeProjects(t).map((p,i)=>`<article class="project-card reveal"><div class="project-visual p${i%3+1}" ${p[4] ? `style="background-image:url('${p[4]}');background-size:cover;background-position:center"` : ''}>${p[4] ? '' : '<div class="mock"><div class="mock-top"></div><div class="mock-side"></div><div class="mock-content"><i></i><i></i><i></i><strong></strong></div></div>'}<span>0${i+1}</span></div><div class="project-body"><small>${p[1]}</small><h3>${p[0]}</h3><p>${p[2]}</p><div class="project-footer"><div class="tags">${p[3].map(x=>`<b>${x}</b>`).join('')}</div>${p[5] ? `<a class="project-link" href="${p[5]}" target="_blank" rel="noopener noreferrer">${lang==='ar'?'عرض المشروع':'View project'} ↗</a>` : ''}</div></div></article>`).join('')}</div></section>
